...
> **SQL TO RUN** (v183 — Project Tracker activity log). Until this is applied the
> Activity Log tab shows "Could not load the activity log" and tracker edits save
> normally but are not recorded.
> ```sql
...
> );
> 
> create index if not exists idx_engagement_log_changed_at
>   on public.engagement_activity_log (changed_at desc);
> create index if not exists idx_engagement_log_engagement
>   on public.engagement_activity_log (engagement_id);
> 
> alter table public.engagement_activity_log enable row level security;
> 
> create policy engagement_activity_log_select_authenticated
>   on public.engagement_activity_log for select to authenticated using (true);
> create policy engagement_activity_log_insert_authenticated
>   on public.engagement_activity_log for insert to authenticated
>   with check (changed_by = current_employee_name());
> -- No UPDATE or DELETE policy on purpose: RLS denies both, so entries are
> -- append-only and cannot be altered, including by a manager.
> ```
>
