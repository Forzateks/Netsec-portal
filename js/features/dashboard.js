...
async function renderDashboard() {
  ...
  var html = '<div style="margin-bottom:20px"><h2 style="font-size:20px;font-weight:700;color:var(--navy)">'+greet+', '+currentUser.split(' ')[0]+' ðŸ‘‹</h2>'+
  ...
  html += '<div class="card" style="margin-bottom:20px"><div class="card-title">Quick Actions</div>'+
    ...
    '<button class="btn btn-primary" onclick="showScreen(\'overtime\');showOTTab(\'log\')">âž• Log OT</button>'+
    '<button class="btn btn-ghost" onclick="showScreen(\'leave\');showLeaveTab(\'log\')">ðŸ–ï¸ Request Leave</button>'+
    '<button class="btn btn-ghost" onclick="showScreen(\'overtime\');showOTTab(\'compoff\')">ðŸ—“ Comp Off</button>'+
    '<button class="btn btn-ghost" onclick="showScreen(\'projects\');showProjectTab(\'log\')">ðŸ“ Log Project Session</button>'+
    (isManager?'<button id="monthly-report-btn" class="btn btn-ghost" onclick="downloadMonthlyReport()">ðŸ“„ Monthly OT Report</button>':'')+
    (isManager?'<button class="btn btn-ghost" onclick="showScreen(\'approvals\')">ðŸ”” Approvals</button>':'')+
    '</div></div>';

  if (pendingCO.length || pendingLV.length || pendingOT.length) {
    html += '<div class="card" style="margin-bottom:20px;border-left:4px solid var(--gold)"><div class="card-title">â³ My Pending Requests</div>';
    pendingOT.forEach(function(r){ html += '<div class="request-card pending" style="margin-bottom:8px">â± OT Session â€” '+r.activity+' on '+fmtDate(r.ot_date)+' ('+r.band+' '+r.duration_hours+'h)<span class="badge badge-pending" style="margin-left:8px">Awaiting Approval</span></div>'; });
    pendingCO.forEach(function(r){ html += '<div class="request-card pending" style="margin-bottom:8px">ðŸ—“ Comp Off â€” '+r.type+' on '+fmtDate(r.request_date)+'<span class="badge badge-pending" style="margin-left:8px">Pending</span></div>'; });
    pendingLV.forEach(function(r){ html += '<div class="request-card pending" style="margin-bottom:8px">ðŸ–ï¸ Leave â€” '+fmtDate(r.start_date)+' â†’ '+fmtDate(r.end_date)+' ('+r.working_days+' days)<span class="badge badge-pending" style="margin-left:8px">Pending</span></div>'; });
    html += '</div>';
  }
  ...
  html += '<div class="card" style="margin-bottom:20px"><div class="flex-between mb-4">'+
    ...
    '<button class="btn btn-sm btn-ghost" onclick="showScreen(\'overtime\');showOTTab(\'sessions\')">View All â†’</button></div>';
  if (recent.length) {
  ...
  } else { html += '<div class="empty-state" style="padding:16px"><div class="empty-icon">ðŸ“­</div><div class="empty-title">No OT sessions yet</div></div>'; }
  html += '</div>';
  ...
  if (isManager) {
    ...
    if (total>0) {
      html += '<div class="card" style="border-left:4px solid var(--gold)"><div class="flex-between">'+
        '<div><div class="card-title" style="margin-bottom:4px">ðŸ”” '+total+' Pending Approvals</div>'+
        '<div style="font-size:13px;color:var(--muted)">'+(approvalResults[0].data||[]).length+' comp off Â· '+(approvalResults[1].data||[]).length+' leave requests</div></div>'+
        '<button class="btn btn-primary" onclick="showScreen(\'approvals\')">Review â†’</button></div></div>';
    }
  }
