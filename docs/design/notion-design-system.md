...
**Saved:** 2026-08-12
**Status:** Reference spec — adopted as the visual direction for NetSec Portal.
**Provenance:** Extracted from Notion's public marketing site (home, Pricing,
Enterprise, Product/AI, Product/Agents, Startups). This is an external design
system captured for reference; NetSec Portal adapts it, it does not clone Notion
or present itself as a Notion product.

> **Implementation note.** This document is the *source* spec, kept verbatim.
> How NetSec Portal actually maps it onto the existing token layer — and where we
> deliberately diverge (numeric mono face, semantic status ramp, OT band colours)
> — is recorded in `docs/design/adoption-notes.md`. Read both before restyling.
