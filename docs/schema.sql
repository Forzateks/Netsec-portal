...
CREATE TABLE public.customers (
  ...
  country text,                              -- v144: UAE / KSA / Oman / … (free text, nullable)
  created_at timestamptz DEFAULT now()
);
...
-- v183: Project Tracker activity log. Append-only (SELECT + INSERT only).
CREATE TABLE public.engagement_activity_log (
  id              bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  engagement_id   bigint,
  engagement_name text,
  changed_by      text NOT NULL,
...
-- v167: Rollout Tracker. Tracks physical SITES against a customer's scope of
-- work — distinct from `engagements`, which tracks delivery phases. Sites are
-- rows so every count in the report is a COUNT and the category breakdown can
-- never drift out of step with the totals.
...
CREATE TABLE public.rollout_sites (
  ...
  country         text NOT NULL,
  city            text,
  -- lowercase with underscores, matching the unified_sessions.session_type
  -- convention; a CHECK rejects capitalised values.
...
-- v171: audit trail for rollout site changes. Same shape as the inventory /
-- amc_contract / ps_deal activity logs. site_id carries NO foreign key on
-- purpose: the log must outlive the site it describes, and site_name/country
-- are denormalised so history stays readable after a rename or a delete.
CREATE TABLE public.rollout_site_activity_log (
  id            bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  site_id       bigint,
  site_name     text,
  country       text,
...
-- v167: Rollout Tracker constraints + indexes.
ALTER TABLE public.rollout_projects ADD CONSTRAINT rollout_projects_pkey PRIMARY KEY (id);
ALTER TABLE public.rollout_sites    ADD CONSTRAINT rollout_sites_pkey    PRIMARY KEY (id);
ALTER TABLE public.rollout_sites    ADD CONSTRAINT rollout_sites_project_fk
  FOREIGN KEY (project_id) REFERENCES public.rollout_projects(id) ON DELETE CASCADE;
ALTER TABLE public.rollout_sites    ADD CONSTRAINT rollout_sites_site_type_check
  CHECK (site_type = ANY (ARRAY['store'::text,'office'::text,'warehouse'::text,
                                'interlinked'::text,'dc_dr'::text,'cloud'::text]));
ALTER TABLE public.rollout_sites    ADD CONSTRAINT rollout_sites_status_check
  CHECK (status = ANY (ARRAY['pending'::text,'done'::text]));
-- A re-run of the import must not silently duplicate the site list.
CREATE UNIQUE INDEX rollout_sites_unique_site        ON public.rollout_sites (project_id, site_name, site_type);
CREATE INDEX idx_rollout_sites_project_country       ON public.rollout_sites (project_id, country);
CREATE INDEX idx_rollout_sites_project_status        ON public.rollout_sites (project_id, status);

ALTER TABLE public.rollout_projects        ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.rollout_sites           ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.rollout_site_activity_log ADD CONSTRAINT rollout_site_activity_log_pkey PRIMARY KEY (id);
CREATE INDEX idx_rollout_log_changed_at ON public.rollout_site_activity_log (changed_at DESC);
CREATE INDEX idx_rollout_log_site       ON public.rollout_site_activity_log (site_id);
ALTER TABLE public.rollout_site_activity_log ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY engagement_activity_log_select_authenticated ON public.engagement_activity_log FOR SELECT TO authenticated USING (true);
CREATE POLICY engagement_activity_log_insert_authenticated ON public.engagement_activity_log FOR INSERT TO authenticated WITH CHECK (changed_by = current_employee_name());
...
-- rollout_projects / rollout_sites (v167) — everyone authenticated reads.
-- Sites are UPDATE-able by anyone: the engineers on site are the ones who know
-- a site is finished, so marking done must be frictionless. Adding, importing
-- and deleting stay manager-only so the SOW baseline cannot move by accident.
CREATE POLICY rollout_projects_select_authenticated ON public.rollout_projects FOR SELECT TO authenticated USING (auth.role() = 'authenticated');
CREATE POLICY rollout_projects_insert_manager       ON public.rollout_projects FOR INSERT TO authenticated WITH CHECK (is_manager_user());
CREATE POLICY rollout_projects_update_manager       ON public.rollout_projects FOR UPDATE TO authenticated USING (is_manager_user());
...
-- with no UPDATE or DELETE policy anywhere, RLS denies both by default, to
-- managers too, so history cannot be rewritten or erased through the API.
-- Unlike the AMC log, INSERT is NOT manager-gated — any engineer can mark a
-- site done, so any engineer must be able to write the matching history.
-- changed_by is bound to current_employee_name() inside the policy, so a
-- caller cannot attribute a change to somebody else.
CREATE POLICY rollout_log_select_authenticated ON public.rollout_site_activity_log FOR SELECT TO authenticated USING (true);
CREATE POLICY rollout_log_insert_authenticated ON public.rollout_site_activity_log FOR INSERT TO authenticated WITH CHECK (changed_by = current_employee_name());
